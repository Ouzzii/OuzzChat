...
function addDivToChatArea(name, message, Year, Hour) {
    ...
    //$datediv.className = "chatArea__text";
    //$datediv.textContent = `${Year}: ${Hour}`;
    //$chatArea.appendChild($datediv);
    // $chatArea.scrollTop = $chatArea.scrollHeight;

    const $messageArea = document.createElement('table');
    $messageArea.className = 'Messages' 

    const $perMessage = document.createElement('th');
    const $perDate = document.createElement('th')

    $perMessage.textContent = `${name}: ${message}`;$perMessage.className = 'messageContent';
    $perDate.textContent = `${Year}: ${Hour}`;$perDate.className = 'messageContent';
    $chatArea.appendChild($messageArea);
    $messageArea.appendChild($perMessage);
    $messageArea.appendChild($perDate);
...
window.addEventListener('DOMContentLoaded', async () => {
    setInterval(async function () {
        ...
        for (let i = 0; i < chatData.length; ++i) {
            addDivToChatArea(chatData[i].name, chatData[i].message, chatData[i].Year, chatData[i].Hour);
        }
